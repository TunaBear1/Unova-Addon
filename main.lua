pokermon.add_family({"tepig", "pignite", "emboar"})
pokermon.add_family({"lillipup", "herdier", "stoutland"})
pokermon.add_family({"timburr", "gurdurr", "conkeldurr"})
pokermon.add_family({"deerling_summer", "deerling_autumn", "deerling_winter", "deerling_spring", "sawsbuck_summer", "sawsbuck_autumn", "sawsbuck_winter", "sawsbuck_spring"})
pokermon.add_family({"cubchoo", "beartic"})

unova_config = SMODS.current_mod.config
      ...
      if curr_pokemon.list and #curr_pokemon.list > 0 then
        for i, item in ipairs(curr_pokemon.list) do
          if (pokermon_config.jokers_only and not item.joblacklist) or not pokermon_config.jokers_only  then
            ...
            pokermon.Pokemon(item, "unova", nil)
          end
        end
      end
    end
